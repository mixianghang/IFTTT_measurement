...
def triggerAlexaBed(resultFile, iterNum = 100, interval = 1, timeLimit = 1000):
    ...
    for index in range(iterNum):
        ...
        triggerTime = datetime.datetime.now()
        print("send out voice command to alexa")
        statStr = "testUuid: {}->triggerGeneratedTime: {}->hueBed".format(index, triggerTime.strftime(datetimeFormat))
        print(statStr)
        resultFd.write(statStr + "\n")
        resultFd.flush()
        time.sleep(interval)
...
def triggerNewGmail(resultFile, iterNum = 100, interval = 1, timeLimit = 1000):
    ...
    while True:
        ...
        triggerTime = datetime.datetime.now()
        statStr = "testUuid: {}->triggerGeneratedTime: {}->gmail_newEmail".format(index, triggerTime.strftime(datetimeFormat))
        print(statStr)
        resultFd.write(statStr + "\n")
        resultFd.flush()
        index += 1
...
def actionHueTurnon(resultFile, iterNum = 100, interval = 1, timeLimit = 1000):
    ...
    while True:
        ...
        if not hueController.isLightOn(lightId):
            ...
            continue
        actionTime = datetime.datetime.now()
        statStr = "testUuid: {}->actionResultObservedTime: {}->hueTurnon".format(index, actionTime.strftime(datetimeFormat))
        print(statStr)
        resultFd.write(statStr + "\n")
        resultFd.flush()
        time.sleep(1)
        ...
        time.sleep(interval)
    resultFd.close()
    print("finish hue turn on action")
...
def actionSwitchTurnon(resultFile, iterNum = 100, interval = 1, timeLimit = 1000):
    ...
    while True:
        ...
        if not  wemoController.isSwitchOn(switchName):
            ...
            continue
        actionTime = datetime.datetime.now()
        statStr = "testUuid: {}->actionResultObservedTime: {}->switchOn".format(index, actionTime.strftime(datetimeFormat))
        print(statStr)
        resultFd.write(statStr + "\n")
        resultFd.flush()
        time.sleep(1)
        ...
        if index >= iterNum:
            break
        time.sleep(interval)
    print("finished wemo turn on action")
    resultFd.close()
def actionHueBlink(resultFile, iterNum = 100, interval = 1, timeLimit = 1000):
    ...
    while True:
        ...
        if  not hueController.isLightAlert(lightId):
            ...
            continue
        actionTime = datetime.datetime.now()
        statStr = "testUuid: {}->actionResultObservedTime: {}->hueBlink".format(index, actionTime.strftime(datetimeFormat))
        print(statStr)
        resultFd.write(statStr + "\n")
        resultFd.flush()
...
recipeTypeDict = {
        "alexa_bed" : (triggerAlexaBed, actionHueTurnon, actionSwitchTurnon),
        "gmail_bed" : (triggerNewGmail, actionHueBlink, actionSwitchTurnon),
}
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("testName")
    parser.add_argument("triggerResultFile")
    parser.add_argument("actionResultFile1")
    parser.add_argument("actionResultFile2")
    ...
    actionResultFile1  = options.actionResultFile1
    actionResultFile2  = options.actionResultFile2
    ...
    if testName not in recipeTypeDict:
        ...
        sys.exit(1)
    triggerFunc, actionFunc1, actionFunc2 = recipeTypeDict[testName]
    triggerProcess = Process(target = triggerFunc, args = (triggerResultFile, iterNum, triggerInterval, timeLimit))
    actionProcess1 = Process(target = actionFunc1, args = (actionResultFile1, iterNum, actionInterval, timeLimit))
    actionProcess2 = Process(target = actionFunc2, args = (actionResultFile2, iterNum, actionInterval, timeLimit))
    overallStartTime = time.time()
    triggerProcess.start()
    actionProcess1.start()
    actionProcess2.start()
    triggerProcess.join()
    actionProcess1.join()
    actionProcess2.join()
